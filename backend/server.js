...
try {
    ...
    // --- [NOVO] ROTAS DE GERENCIAMENTO DE BLOCOS ---
    ...
    app.post('/api/blocks', async (req, res) => {
        ...
        try {
            const blockRef = await db.collection('blocks').add({
                name,
                companyId,
                createdAt: new Date(),
            });
            res.status(201).json({ id: blockRef.id, name, companyId });
        } catch (error) {
    ...
    app.get('/api/blocks', async (req, res) => {
        ...
        try {
            const blocksSnapshot = await db.collection('blocks').where('companyId', '==', companyId).get();
            const blocks = [];
    ...
    app.delete('/api/blocks/:id', async (req, res) => {
        ...
        try {
            ...
            // Também excluiria as associações, mas por simplicidade, vamos deixar assim por enquanto.
    ...
    // Associar fazenda/talhão a um bloco
    app.post('/api/blocks/:id/assign', async (req, res) => {
        const { id } = req.params;
        const { farmId, plotId, companyId } = req.body;
        if (!farmId || !companyId) {
            return res.status(400).json({ message: 'O ID da fazenda e da empresa são obrigatórios.' });
        }
        try {
            await db.collection('block_farm_associations').add({
                blockId: id,
                farmId,
                plotId: plotId || null,
                companyId,
            });
            res.status(200).json({ message: 'Fazenda/talhão associado com sucesso.' });
        } catch (error) {
            console.error("Erro ao associar fazenda/talhão:", error);
            res.status(500).json({ message: 'Erro no servidor ao associar.' });
        }
    });

    // Desassociar fazenda/talhão de um bloco
    app.post('/api/blocks/:id/remove', async (req, res) => {
        const { id } = req.params;
        const { associationId } = req.body;
        if (!associationId) {
            return res.status(400).json({ message: 'O ID da associação é obrigatório.' });
        }
        try {
            await db.collection('block_farm_associations').doc(associationId).delete();
            res.status(200).json({ message: 'Fazenda/talhão desassociado com sucesso.' });
        } catch (error) {
            console.error("Erro ao desassociar fazenda/talhão:", error);
            res.status(500).json({ message: 'Erro no servidor ao desassociar.' });
        }
    });

    // Obter todas as fazendas/talhões de um bloco
    app.get('/api/blocks/:id/farms', async (req, res) => {
        const { id } = req.params;
        try {
            const associationsSnapshot = await db.collection('block_farm_associations').where('blockId', '==', id).get();
            const associations = [];
            ...
            console.error("Erro ao obter fazendas do bloco:", error);
            res.status(500).json({ message: 'Erro no servidor ao obter fazendas do bloco.' });
        }
    });
    ...
    app.get('/reports/block/pdf', async (req, res) => {
        ...
        try {
            const { blockId, companyId, generatedBy } = req.query;
            if (!blockId || !companyId) {
                throw new Error('O ID do bloco e da empresa são obrigatórios.');
            }
            ...
            if (!geojsonData) {
                doc.text('Shapefile da empresa não encontrado. Não é possível gerar o mapa do bloco.');
                generatePdfFooter(doc, generatedBy);
                return doc.end();
            }

            const memberIds = new Set(block.members.map(m => m.id));
            const blockFeatures = geojsonData.features.filter(f => {
                const farmCode = findShapefileProp(f.properties, ['FUNDO_AGR']);
                const talhaoName = findShapefileProp(f.properties, ['CD_TALHAO', 'TALHAO', 'COD_TALHAO', 'NAME']);
                return memberIds.has(`${farmCode}-${talhaoName}`);
            });

            if (blockFeatures.length === 0) {
                doc.text('Nenhum talhão deste bloco foi encontrado no mapa.');
            } else {
                const mapAreaWidth = doc.page.width - (doc.page.margins.left * 2);
                const mapAreaHeight = doc.page.height - currentY - doc.page.margins.bottom - 20;
                ...
                const offsetX = doc.page.margins.left + (mapAreaWidth - (bbox.maxX - bbox.minX) * scale) / 2;
                const offsetY = currentY + (mapAreaHeight - (bbox.maxY - bbox.minY) * scale) / 2;
                ...
                blockFeatures.forEach(feature => {
                    const farmCode = findShapefileProp(feature.properties, ['FUNDO_AGR']);
                    if (!farmColors[farmCode]) {
                        farmColors[farmCode] = colors[colorIndex % colors.length];
                        colorIndex++;
                    }
                    doc.fillColor(farmColors[farmCode], 0.7);
            ...
            currentY = await generatePdfHeader(doc, `Detalhes do Bloco - ${block.name}`, companyId);

            const farmsInBlock = block.members.reduce((acc, member) => {
                if (!acc[member.farmCode]) {
                    acc[member.farmCode] = [];
                }
                acc[member.farmCode].push(member.talhaoName);
                return acc;
            }, {});

            doc.fontSize(12).font('Helvetica-Bold').text('Legenda do Mapa e Composição do Bloco', { underline: true });
            currentY = doc.y + 15;

            for (const farmCode in farmsInBlock) {
                currentY = await checkPageBreak(doc, currentY, title);
                doc.rect(doc.page.margins.left, currentY, 15, 15).fill(farmColors[farmCode] || '#cccccc');
                doc.fontSize(10).font('Helvetica-Bold').text(`Fazenda: ${farmCode}`, doc.page.margins.left + 25, currentY + 4);
                doc.font('Helvetica').text(`Talhões: ${farmsInBlock[farmCode].sort().join(', ')}`, { indent: 25 });
                currentY = doc.y + 10;
            }
